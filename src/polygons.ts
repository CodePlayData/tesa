...
async function belongsTo(
  ...
  type: string,
  config?: Config,
): Promise<undefined | LocationInfo> {
  const url = {
    country: config?.lists?.main?.country ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/country_list.csv",
    macroregion: config?.lists?.main?.macroregion ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/macroregion_list.csv",
    states: config?.lists?.main?.states ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/states_list.csv",
    middleregions: config?.lists?.main?.middleregions ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/middlewareregion_list.csv",
    immediate: config?.lists?.main?.immediate ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/immediate_list.csv",
    microregions: config?.lists?.main?.microregions ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/microregion_list.csv",
    intermediary: config?.lists?.main?.intermediary ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/intermediary_list.csv",
    cities: config?.lists?.main?.cities ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/cities_list.csv",
  } as { [key: string]: string };

  const doublesUrl = {
    microregions: config?.lists?.doubles?.microregions ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/micro_double_list.csv",
    immediate: config?.lists?.doubles?.immediate ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/immediate_doubles.csv",
    cities: config?.lists?.doubles?.cities ??
      "https://raw.githubusercontent.com/CodePlayData/tesa/main/src/data/cities_double_list.csv",
  } as { [key: string]: string };
  ...
  try {
    ...
    const place = list.filter((place) =>
      place.Alias ===
        alias.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    )[0];
    ...
    if (config) {
      location = {
        ...
        middleregions: `${config?.api?.localidades?.middleregions}${place
          ?.Code}`,
      ...
      location = {
        cities:
          `https://servicodados.ibge.gov.br/api/v1/localidades/municipios/${place.Code}`,
        microregions:
          `https://servicodados.ibge.gov.br/api/v1/localidades/microrregioes/${place.Code}`,
        intermediary:
          `https://servicodados.ibge.gov.br/api/v1/localidades/regioes-intermediarias/${place.Code}`,
        middleregions:
          `https://servicodados.ibge.gov.br/api/v1/localidades/mesorregioes/${place.Code}`,
        states:
          `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${place.Code}`,
        immediate:
          `https://servicodados.ibge.gov.br/api/v1/localidades/regioes${place.Code}`,
      };
    }
...
async function belongsToMany(
  ...
  if (config) {
    baseUrl = {
      ...
      intermediary: `${config?.api?.localidades
        ?.states}${code}/regioes-intermediarias`,
  ...
  if (type === "microregions" || type === "cities" || type === "immediate") {
    try {
      ...
      const doubles = aliases.map((i) =>
        //tem que ver se esse i é reconhecido como string, se sim typagem ok té aqui
  ...
  if (type === "macroregion" || type === "states") {
    try {
      const statesJson: LocationInfo[] = JSON.parse(
        await (await fetch(baseUrl[type] as unknown as Request)).text(),
      );
      const list = await parseCsv(await (await fetch(url[type])).text(), {
        skipFirstRow: true,
        separator: ";",
      }) as ListItems[];
    ...
    try {
      ...
      result.map((i) => place_codes.push(i.toString().substring(0, 2)));
      unique_codes = [...new Set(place_codes)] as string[];
      const major_polygons: LocationInfo[] = await Promise.all(
        unique_codes.map(async (i) => {
          code = eval(i);
          if (config) {
            switch (type) {
              case "middleregions":
                baseUrl = `${config?.api?.localidades
                  ?.states}${code}/mesorregioes`;
                break;
              case "intermediary":
                baseUrl = `${config?.api?.localidades
                  ?.states}${code}/regioes-intermediarias`;
              ...
              case "immediate":
                baseUrl = `${config?.api?.localidades
                  ?.states}${code}/regioes-imediatas`;
                break;
              case "microregions":
                baseUrl = `${config?.api?.localidades
                  ?.states}${code}/microrregioes`;
                break;
              case "cities":
                baseUrl = `${config?.api?.localidades
                  ?.states}${code}/municipios`;
                break;
            }
          } else {
            switch (type) {
              case "middleregions":
                baseUrl =
                  `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${code}/mesorregioes`;
                break;
              case "intermediary":
                baseUrl =
              ...
              case "microregions":
                baseUrl =
                  `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${code}/microrregioes`;
                break;
              case "cities":
                baseUrl =
          ...
          return JSON.parse(
            await (await fetch(baseUrl as unknown as Request)).text(),
          );
        }),
      );
 ...
 */
async function getOnePolygon(
  alias: string,
  type: string,
  config?: Config,
): Promise<undefined | GeoJsonObject> {
  ...
  try {
    ...
    const RequestUrl = list.filter((place) =>
      place.Alias === alias
        .toUpperCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
 ...
 */
async function getManyPolygons(
  request: MetaInfoRequest,
  config?: Config,
): Promise<undefined | LocationInfo[]> {
  ...
  try {
    if (type === "microregions" || type === "cities" || type === "immediate") {
      try {
        ...
        const doubles: string[][][] = aliases.map((i) =>
          //tem que ver se esse i é reconhecido como string, se sim typagem ok té aqui
          doublesList.filter((place) =>
            place.Alias === i
              .toUpperCase()
        ...
        if (doublesResult.length > 0) {
          console.log(
            "There are repeated names in this geographic category, try to replace for:\n",
          );
          doublesResult.flat().map((i) => console.log(`${String(i)}`));
          return;
        }
    ...
    if (type === "macroregion" || type === "states") {
      if (config) {
        ...
        baseUrl = {
          macroregion:
            "https://servicodados.ibge.gov.br/api/v3/malhas/paises/BR?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=regiao",
          states:
            "https://servicodados.ibge.gov.br/api/v3/malhas/paises/BR?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=UF",
        };
      }

      const statesJson: GeoJsonObject = JSON.parse(
        await (await fetch(baseUrl[type] as unknown as Request)).text(),
      );
      ...
      const major_polygons = await Promise.all(unique_codes.map(async (i) => {
        ...
        if (config) {
          baseUrl = {
            middleregions: `${config?.api?.malhas
              ?.states}${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=mesorregiao`,
            microregions: `${config?.api?.malhas
              ?.states}${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=microrregiao`,
            cities: `${config?.api?.malhas
              ?.states}${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=municipio`,
          };
        } else {
          baseUrl = {
            middleregions:
              `https://servicodados.ibge.gov.br/api/v3/malhas/estados/${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=mesorregiao`,
            microregions:
              `https://servicodados.ibge.gov.br/api/v3/malhas/estados/${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=microrregiao`,
            cities:
              `https://servicodados.ibge.gov.br/api/v3/malhas/estados/${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=municipio`,
          };
        }
...
async function getNeighborhoodsFromCity(city: string) {
  const normalizedCity = city.replace(/\s+/g, "").normalize("NFD").replace(
    /[\u0300-\u036f]/g,
    "",
  );
