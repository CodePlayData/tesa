...
async function belongsTo(alias, type) {
  ...
  if (type === "microregions" || type === "cities" || type === "immediate") {
    const doubles_list = await parseCsv(
      await (
    ...
    const doubles = doubles_list.filter((place) =>
      place.Alias === alias
        .toUpperCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
    ...
    if (doubles.length > 0) {
      console.log(
        "Existem nomes repetidos nessa categoria geográfica, experimente trocar para:\n",
      );
  ...
  const result = await fetch(
    ...list.filter((place) =>
      ...
      .map((place) => {
        switch (type) {
          case "cities":
            return `${config.api.localidades.cities}${place.Code}`;
          case "microregions":
            return `${config.api.localidades.microregions}${place.Code}`;
          case "intermediary":
            return `${config.api.localidades.intermediary}${place.Code}`;
          case "middleregions":
            return `${config.api.localidades.middleregions}${place.Code}`;
          case "states":
            return `${config.api.localidades.states}${place.Code}`;
          case "immediate":
            return `${config.api.localidades.immediate}${place.Code}`;
        }
      }),
  );
...
async function belongsToMany(request) {
  ...
  if (type === "microregions" || type === "cities" || type === "immediate") {
    ...
    let doubles = aliases.map((i) =>
      doubles_list.filter((place) =>
        place.Alias === i
          .toUpperCase()
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
  ...
  if (type === "macroregion" || type === "states") {
    ...
    metaInfos = [
      ...result.map((i) => states_json.filter((o) => o.id === eval(i))).flat(),
    ];
    return (metaInfos);
  } else {
    // get the alias list
    ...
    metaInfos = [
      ...result.map((i) =>
        major_polygons.map((o) => o.filter((u) => u.id === eval(i)))
      ).flat().flat(),
    ];
...
async function getManyPolygons(request) {
  ...
  try {
    ...
    if (type === "macroregion" || type === "states") {
      ...
      polygons = [
        ...result.map((i) =>
          states_json.features.filter((o) => o.properties.codarea === i)
        ).flat(),
      ];
      return (polygons);
    } else {
      ...
      const major_polygons = await Promise.all(unique_codes.map(async (i) => {
        ...
        switch (type) {
          case "middleregions":
            base_url =
              `${config.api.malhas.states}${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=mesorregiao`;
            break;
          case "microregions":
            base_url =
              `${config.api.malhas.states}${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=microrregiao`;
            break;
          case "cities":
            base_url =
              `${config.api.malhas.states}${code}?formato=application/vnd.geo+json&qualidade=maxima&intrarregiao=municipio`;
            break;
        }
      ...
      polygons = [
        ...result.map((i) =>
          major_polygons.map((o) =>
            o.features.filter((u) => u.properties.codarea === i)
          )
        ).flat().flat(),
      ];
