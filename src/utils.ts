 ...
 */
 import { tgz, readerFromStreamReader } from "../deps.ts";
 ...
 export async function getOsmExtractTools() {
     const urls = {
         extract_osm: "https://github.com/AndGem/osm_extract_polygon/releases/download/v.0.3.3/osm_extract_polygon-linux-amd64.tar.gz",
     ...
     console.log('Now install osmium-tools with:\n "sudo apt install osmium-tool"')
 };
 ...
 export async function getCityPbf(city: string) {
     ...
     try {
         ...
         const City:string = city.replace(/\s+/g, '').normalize("NFD").replace(/[\u0300-\u036f]/g, "")
         
         const url = {
             Sul: "https://download.geofabrik.de/south-america/brazil/sul-latest.osm.pbf",
             Sudeste: "https://download.geofabrik.de/south-america/brazil/sudeste-latest.osm.pbf",
             Norte: "https://download.geofabrik.de/south-america/brazil/norte-latest.osm.pbf",
             Nordeste: "https://download.geofabrik.de/south-america/brazil/nordeste-latest.osm.pbf",
             'Centro-Oeste': "https://download.geofabrik.de/south-america/brazil/centro-oeste-latest.osm.pbf"
         } as { [key: string]: string };
     
         await Promise.all(
             [
                 await Deno.writeTextFile(`${City}.geojson`, JSON.stringify(polygon)),
                 await downloadFile(url[metadata.microrregiao.mesorregiao.UF.regiao.nome],`./${metadata.microrregiao.mesorregiao.UF.regiao.nome}-latest.osm.pbf`),               
                 await runCmd(`./src/bin/osmium extract --strategy smart -p ${City}.geojson ${metadata.microrregiao.mesorregiao.UF.regiao.nome}-latest.osm.pbf -o ${City}.pbf`)
             ]
         ) 
     } catch (error) {
 ...
 export async function extractNeighboorhodFromPbf(path: string) {
     await runCmd(`./src/bin/osm_extract_polygon -f ${path} --geojson -o --max 10 --min 10`, false)
 };
 ...
 export async function readNeighborhoodGeojsonFromDir(path: string) {
     ...
     for await (const dirEntry of Deno.readDir(path)) {
         if(dirEntry.isFile && dirEntry.name.match(".geojson$")){
             features.push(await JSON.parse(await Deno.readTextFile(`./${path}/${dirEntry.name}`)))
         }
       }
